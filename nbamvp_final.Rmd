...
=======
  ...
  filter(year>1999) %>%
>>>>>>> 5ee6c02a883a402706772b084446fea4bbbc57a3
...
final_train$allstar <- as.numeric(final_train$allstar)
final_test$allstar <- as.numeric(final_test$allstar)
...
cv_reg %>% collect_metrics() %>% arrange(mean)
#finalize 

enlinreg_final <- workflow_enlinreg%>%
  finalize_workflow(select_best(cv_reg, metric="rmse"))
...
best_flow_reg <- workflow_enlinreg %>%
  finalize_workflow(select_best(cv_reg, metric = "rmse")) %>%
  fit(data = final_train)

#pull the best regression model
best_reg <- best_flow_reg %>% pull_workflow_fit()

#Fitting the final model
final_fit_reg = best_flow_reg %>% fit(data = final_train)

#Predict onto the test data
y_hat_reg = final_fit_reg %>% predict(new_data = final_test, type ="numeric")

final_test$allstar_reg <- y_hat_reg$.pred
...
#add predictions to testing df
final_test$allstar_pred <- allstar_pred_vec

#generate confusion matrix
cm_logistic_reg = conf_mat(
  data = tibble(
    y_hat = final_test$allstar_pred %>% unlist() %>% as.factor(),
    y = as.factor(final_test$allstar - 1)
  ),
  truth = y, estimate = y_hat
)

#plot confusion matrix
cm_logistic_reg$table %>% as.data.frame() %>%
    mutate(result = c("True Negative","False Positive","False Negative","True Positive")) %>%
    filter(!result == "True Negative") %>% 
  ggplot(aes(x = result, y = Freq)) + geom_bar(stat = "identity")
  geom_text(aes(x = "False Negative",y = c(600,750,900,1050),label = paste(result,"s: ",Freq,sep = "")))
```
