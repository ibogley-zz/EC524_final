...
best_flow_reg <- workflow_en %>%
  finalize_workflow(select_best(cv_en, metric = "rmse")) %>%
  fit(data = final_train)

#pull the best regression model
best_tree <- best_flow_reg %>% pull_workflow_fit()
...
final_test$allstar_prob <- y_hat_reg$.pred
...
for (i in unique(final_test$year)) {
  cutoff <- final_test[final_test$year == i,] %>% pull(allstar_prob) %>%
    sort(decreasing = TRUE) %>%
    .[25]
  final_test[final_test$year == i,] %>%
    mutate(allstar_pred = ifelse(allstar_prob>=cutoff,1,0))
}
